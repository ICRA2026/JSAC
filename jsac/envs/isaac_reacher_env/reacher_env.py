...
class CarterReacherEnv(gymnasium.Env):
    def __init__(self, scene_path, seed=0, min_target_size=0.8, physics_dt=1.0/25.0, rendering_dt = 1.0/25.0, 
        ...
        self._v_w_low = np.array([-0.4, -0.4])
    ...
    def get_randon_poses(self):
        ...
        if part == 1:
            carter_x = np.random.uniform(low=1.5, high=3.25)
        ...
        if dist > 0.5:
            or1 = np.random.uniform(low=-179, high=179)
