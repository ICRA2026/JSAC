...
class ActorModel(nn.Module):
    ...
    def __init__(
        ...
        first_layer = self.encoder.latent_dim
        final_layer = action_dim * 2

        last_layer = first_layer

        layers = []
        for hidden_size in mlp_params:
            layers.append(nn.Linear(last_layer, hidden_size))
            last_layer = hidden_size
            layers.append(nn.ReLU())
        layers.append(nn.Linear(last_layer, final_layer))
...
class QFunction(nn.Module):
    ...
    def __init__(self, latent_dim, action_dim, net_params):
        ...
        first_layer = latent_dim + action_dim
        final_layer = 1
        last_layer = first_layer
